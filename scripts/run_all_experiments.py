    ...
    for seed in SEEDS:
        for exposure in EXPOSURES:
            for condition in CONDITIONS:
                ...
                # 1. Train Model (Skip if already trained)
                trained_model_dir = MODELS_DIR / model / f"seed_{seed}" / f"exposure_{exposure}" / condition
                if (trained_model_dir / "config.json").exists():
                    print(f"Model already trained at {trained_model_dir}. Skipping training step...")
                else:
                    train_cmd = [
